    ...
    <div class="container m-auto">
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <select v-model="selected" v-for="(parent,index) in parentCategories" class="form-select" size="3" aria-label="size 3 select example">
            <option v-bind:value="{ parent: parent }">{{parent.name}}</option>
          </select>
        </div>
