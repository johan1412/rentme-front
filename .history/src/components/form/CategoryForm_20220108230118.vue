    ...
    <div class="container m-auto">
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <select v-model="parent" class="form-select form-select-lg mb-3" aria-label="size 3 select example" v-for=" item in parentCategories" :key="item.id">
            <option value="">{{item.name}}</option>
          </select>
          <select class="form-select form-select-lg mb-3" v-if="parentCategories.length < 1" aria-label="size 3 select example">
