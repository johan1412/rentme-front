    ...
    <div class="container m-auto">
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <select v-model="parents" class="form-select" size="3" aria-label="size 3 select example" v-for=" parent in parentCategories" :key="parent.id">
            <option value="{{id:parent.id}}">{{parent.name}}</option>
          </select>
        </div>
