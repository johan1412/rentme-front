    ...
    <div class="container m-auto">
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <select v-model="parentSelected" class="form-select" size="3" aria-label="size 3 select example" v-for=" parent in parentCategories" :key="parent.id" v-if="parentCategories.length > 1">
            <option value="">{{parent.name}}</option>
          </select>
          <select class="form-select" size="1" v-else aria-label="size 3 select example">
